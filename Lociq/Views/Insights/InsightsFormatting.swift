...
enum InsightsFormatting {
    ...
    nonisolated static func currency(_ value: Int?) -> String {
        guard let value else { return AppStrings.Symbols.emDash }
        return NumberFormatting.currencyString(value)
    }

    nonisolated static func percent(_ value: Double?, suffixCount: Int?) -> String {
        guard let value else { return AppStrings.Symbols.emDash }
    ...
    nonisolated static func normalizedPercent(_ value: Double?) -> Double {
        guard let value else { return 0 }
        return max(0, min(1, value / 100))
    }

    nonisolated static func demographicShare(_ value: Int?, totalPopulation: Int?) -> Double {
    ...
    nonisolated static func dataSourceText(_ source: MetricsSource) -> String {
        switch source {
