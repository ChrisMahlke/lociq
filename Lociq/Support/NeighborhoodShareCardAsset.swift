...
struct ComparisonShareCardAsset: Transferable {
    let title: String
    let summary: String
    let imageData: Data
    ...
    static var transferRepresentation: some TransferRepresentation {
        FileRepresentation(exportedContentType: .png) { asset in
            let fileName = asset.title
                ...
                .replacingOccurrences(of: "[^a-z0-9]+", with: "-", options: .regularExpression)
                .trimmingCharacters(in: CharacterSet(charactersIn: "-"))
            let safeName = fileName.isEmpty ? "lociq-comparison-card" : "lociq-\(fileName)"
            let url = FileManager.default.temporaryDirectory
                .appendingPathComponent("\(safeName)-\(UUID().uuidString).png")
...
@MainActor
enum ComparisonShareCardExporter {
    static func makeAsset(
        boundaryScale: BoundaryOverlayScale,
        primary: ComparablePlaceProfile,
        secondary: ComparablePlaceProfile
    ) -> ComparisonShareCardAsset? {
        let title = "\(primary.title) \(AppStrings.Labels.compareVersus) \(secondary.title)"
        ...
        let content = ComparisonShareCardView(
            title: title,
            boundaryScale: boundaryScale,
            primary: primary,
            secondary: secondary
        ...
        return ComparisonShareCardAsset(
            title: title,
            summary: summary,
            imageData: imageData
        )
...
private struct ComparisonShareCardView: View {
    let title: String
    let boundaryScale: BoundaryOverlayScale
    let primary: ComparablePlaceProfile
    let secondary: ComparablePlaceProfile

    private var rows: [ComparisonShareMetricRow] {
        [
            ComparisonShareMetricRow(
        ...
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 18) {
                HStack(alignment: .top) {
                    ...
                    NeighborhoodSharePill(
                        text: boundaryScale.displayTitle,
                        tint: boundaryScale.themeColor
                    )
                }

                HStack(spacing: 12) {
                    ComparisonSharePlacePanel(
                        title: primary.title,
                    ...
                    ComparisonShareVersusPill()

                    ComparisonSharePlacePanel(
                        title: secondary.title,
                        subtitle: secondary.subtitle,
                        sourceText: InsightsFormatting.dataSourceText(secondary.metricsSource),
                        tint: .orange
                    )
                }
...
private struct ComparisonShareMetricRow: Identifiable {
    let id = UUID()
    let label: String
...
private struct ComparisonSharePlacePanel: View {
    let title: String
    let subtitle: String
...
private struct ComparisonShareVersusPill: View {
    var body: some View {
...
private struct ComparisonShareMetricPanel: View {
    let row: ComparisonShareMetricRow
...
private struct ComparisonShareMetricValue: View {
    let title: String
    let value: String
    let tint: Color
